CDN Beefnet version 0.13.2 is now available from:

  <https://github.com/canada-ecoin/beefnet/releases>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/canada-ecoin/beefnet/issues>
...
updates will be released anymore. Without security updates, using a beefnet
...
Previous versions of CDN Beefnet had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of CDN Beefnet was to raise the default minimum
relay fee.

CDN Beefnet 0.13.2 will have a strict maximum size on the mempool. The
...
CDN Beefnet 0.13.2 also introduces new default policy limits on the length and
...
This allows running beefnetd without having to do any manual configuration.
...
CDN Beefnet has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. CDN Beefnet will relay transactions with
...
entirely in CDN Beefnet 0.13, and it is at this time undecided whether the
...
CDN Beefnet has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
CDN Beefnet automatically creates a hidden service to listen on, without
manual configuration. CDN Beefnet will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if CDN Beefnet is listening, and
...
Beefnetd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Beefnet
to use floating fees. By default, CDN Beefnet will use floating fees.
...
(default: `0.0002` BEEF/kB).

At all times, CDN Beefnet will cap fees at `-maxtxfee=<x>` (default:
0.10) BEEF.
Furthermore, CDN Beefnet will never create transactions paying less than
...
`beefnet.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /beefnetrpc>
        ...
        # optional bypass beefnetd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< beefnetrpc:<password>
    </Location>
...
Restarting beefnetd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in beefnet.conf
...
For example:
    ...
    $ src/beefnet-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The CDN Beefnet code base has
...
- `beefnet-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `beefnet-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
HD wallets are incompatible with older versions of CDN Beefnet.
...
Due to Segregated Witness, the mining algorithm has been modified
to optimize transaction selection for a given block weight, rather than a given
number of serialized bytes in a block.
...
`-blockmaxsize` option may be removed, as block creation is
...
The two phases are distinct in the Beefnet-Qt GUI. During the first one,
...
Segregated witness (segwit) is a soft fork that will
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of BEEF they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of BEEF being spent is exactly
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  proof of work.  Segwit enables this capability at the consensus layer.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Canada eCoin users to individually opt-in to almost any change in the Canada eCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and CDN Beefnet contributors that may
...
Since CDN Beefnet 0.10.0, nodes have defaulted to only relaying and
...
- `beefnet-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `beefnet-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
