...
*Setup instructions for a Gitian build of CDN Beefnet using a Debian VM or physical system.*
...
- [Building CDN Beefnet](#building-beefnet)
...
Clone the git repositories for beefnet and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/canada-ecoin/beefnet
...
in the beefnet repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building CDN Beefnet
----------------

To build CDN Beefnet (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the beefnet repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/beefnet/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/canada-ecoin/beefnet
...
For example:
```bash
URL=https://github.com/canada-eoin/beefnet.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit beefnet=${COMMIT} --url beefnet=${URL} ../beefnet/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit beefnet=${COMMIT} --url beefnet=${URL} ../beefnet/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit beefnet=${COMMIT} --url beefnet=${URL} ../beefnet/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the beefnet git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../beefnet/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/beefnet
SIGPATH=/some/root/path/beefnet-detached-sigs

./bin/gbuild --url beefnet=${BTCPATH},signature=${SIGPATH} ../beefnet/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/beefnet-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/beefnet-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/beefnet-osx-build.assert
```
